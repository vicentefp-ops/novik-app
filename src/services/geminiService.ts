...
async function processCitationsAndReferences(text: string, language: string): Promise<string> {
  ...
  if (refContentMap.size === 0) {
    // Fallback: if we couldn't parse the references, just return the original text
    const header = language === 'en' ? 'References' : 'Referencias bibliográficas';
    const trimmedRefsText = refsText.trim();
    
    if (!trimmedRefsText) {
      const fallbackMsg = language === 'en' 
        ? 'No specific bibliographic references were found for this case.' 
        : 'No se encontraron referencias bibliográficas específicas para este caso.';
      return `${mainText.trim()}\n\n### ${header}\n${fallbackMsg}`;
    }
    
    const verifiedRefsText = await verifyAndFixPubMedLink(trimmedRefsText);
    return `${mainText.trim()}\n\n### ${header}\n${verifiedRefsText}`;
  }
